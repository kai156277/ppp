...
void math_function::sunPostion(double juliday,double *posCTS)
{
    /*--------------------------------------------------------------------------
    ...
    getSunPosCIS(juliday);
    CIS2CTS(posCTS,juliday);
}

void math_function::moonPostion(double juliday,double *posCTS)
{
    /*--------------------------------------------------------------------------
     * Returns the position of Moon ECEF coordinates (meters) at the indicated time.
     * t the time to look up
     * return the position of the Moon at time.
     * -------------------------------------------------------------------------*/

    /*Test if the time interval is correct--------------------------------------*/
    if( juliday < 2415077.5 || juliday > 2488129.5 )
    {
        std::cerr() << "Provided epoch is out of bounds.";
        exit(1);
    }

    spaceRectangular posCIS=getMoonPosCIS(juliday);
    spaceRectangular posCTS=CIS2CTS(posCIS,juliday);
    return posCTS;

}

void math_function::getSunPosCIS(double juliday,double *posCIS)
{
    ...
    gregorianTime t1;
    dayofYear t2;
    timeTransform::julianToGregorian(juliday,t1);
    timeTransform::gregorianToDOY(t1,t2);
    ...
    spaceRectangular sunposCIS;
    sunposCIS.X = (r*coselt + MeanEarthMoonBary*celmm) * AU_CONST;
    sunposCIS.Y = (MeanEarthMoonBary*selmm - w1) * coseps * AU_CONST;
    sunposCIS.Z = (-w1*sineps) * AU_CONST;
    return sunposCIS;
}

void math_function::getMoonPosCIS(double juliday)
{
    ...
    double mjd = juliday.day - 2400000.5;
    ...
    spaceRectangular monposCIS;
    monposCIS.X = (X-EC*Y+ES*Z) * AU_CONST;
    monposCIS.Y = (EQCOR*X+Y*COSEPS-Z*SINEPS) * AU_CONST;
    monposCIS.Z = (Y*SINEPS+Z*COSEPS) * AU_CONST;

    return monposCIS;
}

void math_function::CIS2CTS(const spaceRectangular &posCIS, double juliday)
{
    gregorianTime t1;
    dayofYear t2;
    timeTransform::julianToGregorian(juliday,t1);
    ...
    spaceRectangular posCTS;
    posCTS.X =  cos(ts)*posCIS.X + sin(ts)*posCIS.Y;
    posCTS.Y = -sin(ts)*posCIS.X + cos(ts)*posCIS.Y;
    posCTS.Z = posCIS.Z;
    return posCTS;
}
